...
if missing_columns:
    ...
    # Aggregate the data by summing up COVID-19 cases per area and town
    ...
    # Check if there are any cases to avoid division by zero
    if agg_df['Nb of Covid-19 cases'].sum() == 0:
        st.error("No COVID-19 cases to display.")
    else:
        ...
        if total_cases == 0:
            st.error("No cases to display in the pie chart.")
        else:
            if show_percentage:
